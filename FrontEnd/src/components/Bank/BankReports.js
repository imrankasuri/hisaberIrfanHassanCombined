...
const BankReports = () => {
  ...
  const fetchBankTransactions = async (FormData) => {
    ...
    try {
      const api_config = {
        ...
        url: `${
          Config.base_url
        }Reports/GetBankReportBy/${CompanyID}?bankCode=${bankCode}&startDate=${effectiveStartDate}&endDate=${effectiveEndDate}&period=${
          FormData.period
  ...
  useEffect(() => {
    ...
    if (selectedBank) {
      BankForm.setFieldValue("bankName", selectedBank.accountDescription);
      setSelectedBank(selectedBank);
    }
  }, [ListOfRecords, bankCode]);
  ...
  let runningBalance = 0; // Running balance for the table
  const columns = [
    {
      title: "Sr#",
      dataIndex: "",
...

    {
      ...
      render: (text, record) => {
        ...
        let reportType = "account"; // default
...

    {
      ...
      render: (text, record, index) => {
        ...
        return (
          <div style={{ textAlign: "right" }}>{runningBalance.toFixed(2)}</div>
        );
      },
  ...
  const summary = () => {
    return (
      <Table.Summary.Row style={{ textAlign: "right" }}>
        ...
        <Table.Summary.Cell>{TotalPayment.toFixed(2)}</Table.Summary.Cell>
        <Table.Summary.Cell>{TotalReceipt.toFixed(2)}</Table.Summary.Cell>
