...
const EditBankReceipts = () => {
  ...
  useEffect(() => {
    document.title = "Edit Bank Receipts";
  ...
  const fetchBankPaymentData = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        Config.base_url + `Bank/GetBankReceiptByVoucher`,
        data,
        {
          headers: {
      ...
      if (response.data.status_code === 1) {
        setBankPaymentList(response.data.bankReceiptData);
        PaymentForm.setFieldsValue({ users: response.data.bankReceiptData });
        form.setFieldsValue({ Bank: response.data.bankReceiptData[0].bank });
        setOpenDate(response.data.bankReceiptData[0].date);
        setSelectedBank(response.data.bankReceiptData[0].bankCode);
  ...
  const onFinish = async (value) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${Config.base_url}Bank/UpdateRecord`,
        data,
        {
          headers: {
            Authorization: `Bearer ${AccessKey}`,
...
export default EditBankReceipts;
