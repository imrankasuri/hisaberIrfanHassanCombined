...
const EditReturnReceipt = () => {
          ...
          if (ProductResponse.data.status_code === 1) {
            const updateCustomerData = {
              ...
              customerOpeningBalance:
                parseInt(selectedCustomer.customerOpeningBalance) +
  ...
  useEffect(() => {
    document.title = "Edit Sale Return Receipt";
    fetchCustomerData();
    fetchBankAccounts();
    fetchBankMode();
    ReceiptBody();
  ...
  const fetchCustomerData = async () => {
    ...
    try {
      ...
      if (data.status_code === 1) {
        ...
        const apiUrl = `${Config.base_url}ReceiptHead/GetReceiptBy/${CompanyID}?customerCode=${code}&receiptType=Receipt`;
...
export default EditReturnReceipt;
