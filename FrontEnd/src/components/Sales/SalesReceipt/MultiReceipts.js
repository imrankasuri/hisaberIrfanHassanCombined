...
function MultiReceipt() {
  ...
  const [submitting, setSubmitting] = useState(false); // New state for submission progress
  const [submitProgress, setSubmitProgress] = useState(0); // New state for progress percentage
    ...
    try {
      ...
      if (response.data.status_code === 1) {
        ...
        message.success(
          <>
            {receipt.map((item) => (
              <div className="alert-head" key={item.voucherNo}>
