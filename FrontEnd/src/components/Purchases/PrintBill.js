...
const PrintBill = () => {
  ...
  const handlePrint = useReactToPrint({
    contentRef,
    documentTitle: `Purchase Bill - ${params.id}`,
  ...
  const fetchSales = async () => {
    ...
    try {
      ...
      if (response.data.status_code === 1) {
        ...
        setTotalPurchase(response.data.purchaseHead);
        ////Console.log(response.data.purchaseHead);
        setPurchase(response.data.listofPurchaseBody);
      }
    } catch (error) {
  ...
  useEffect(() => {
    document.title = "Print Bill";
    fetchSales();
  }, []);
  ////Console.log(totalPurchase);

  useEffect(() => {
    if (purchase.length > 0 && totalPurchase) {
        ...
        <div ref={contentRef} className="print-content px-5 py-3">
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  <strong>Supplier:</strong> {totalPurchase.supplierName}
                </p>
                <p
                  style={{
                    margin: "5px 0",
                ...
                >
                  <strong>Bill No:</strong> {params.id}
                </p>
                <p
                  style={{
                    margin: "5px 0",
                ...
                >
                  <strong>Date:</strong> {totalPurchase.date}
                </p>
              </div>
          ...
          >
            ...
            <Col span={12} style={{ textAlign: "right" }}>
              {totalPurchase.subTotal} <br />
              {totalPurchase.totalDiscount} <br />
              {totalPurchase.totalSaleTax} <br />
              {totalPurchase.overallDiscount} <br />
              <b>{totalPurchase.total}</b>
            </Col>
          </Row>
