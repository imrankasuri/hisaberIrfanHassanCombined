...
const AddReturnSupplierReceipt = () => {
      ...
      if (response.data.status_code === 1) {
        ...
        try {
          ...
          if (ProductResponse.data.status_code === 1) {
            ...
            message.success(
              <>
                Return Receipt Added Successfully Against <br />
  ...
  useEffect(() => {
    document.title = "Add Supplier Return Receipt";
  ...
  const handleSupplierChange = async (value) => {
    ...
    const apiUrl = `${Config.base_url}PaymentHead/GetPaymentBy/${CompanyID}?supplierCode=${code}&purchaseType=Receipt`;
...
export default AddReturnSupplierReceipt;
